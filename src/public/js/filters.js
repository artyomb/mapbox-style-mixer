class Filters {
  ...
  updateFilterButtons() {
    Object.keys(this.filterStates).forEach(filterId => {
      const groupButton = document.getElementById(`filter-${filterId}`);
      if (groupButton) {
        groupButton.className = `control-button ${this.filterStates[filterId] ? 'active' : 'inactive'} filter-group-button`;
      }
    ...
    Object.keys(this.currentStyle.metadata.filters).forEach(filterId => {
      ...
      if (filterConfig.length > 1) {
        ...
        filterConfig.forEach(item => {
          ...
          this.filterStates[`${filterId}_${item.id}`] = true;
        });
